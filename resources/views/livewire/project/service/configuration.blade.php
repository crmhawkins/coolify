    ...
    <div class="flex flex-col h-full gap-8 sm:flex-row">
        <div class="sub-menu-wrapper">
            ...
            @if ($this->hasWordPress())
                <a class='sub-menu-item' wire:current.exact="menu-item-active" {{ wireNavigate() }}
                    href="{{ route('project.service.wordpress-manager', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}"><span class="menu-item-label">WordPress Manager</span></a>
            @endif
            @if ($this->hasLaravel())
                <a class='sub-menu-item' wire:current.exact="menu-item-active" {{ wireNavigate() }}
                            ...
                            <div class="flex flex-row w-full">
                                ...
                                <div class="flex items-center px-4">
                                    ...
                                    @if (str($application->status)->contains('running'))
                                        @if ($this->hasWordPress() && str_contains(strtolower($application->image ?? ''), 'wordpress'))
                                            <a href="{{ route('project.service.wordpress-manager', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}"
                                                class="button bg-blue-600 mr-2">
                                        ...
                                        @endif
                                        @if ($this->hasLaravel() && (str_contains(strtolower($application->image ?? ''), 'laravel') || str_contains(strtolower($application->image ?? ''), 'php')))
                                            <a href="{{ route('project.service.laravel-manager', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'service_uuid' => $service->uuid]) }}"
                                                class="button bg-red-600 mr-2">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path>
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                </svg>
