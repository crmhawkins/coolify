            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex gap-2 items-end flex-wrap">
                    ...
                    <div class="flex items-center gap-2">
                        <x-forms.button wire:click="checkUnzip" wire:loading.attr="disabled" class="bg-green-600">
                ...
                @if ($selected_container !== 'default')
                    ...
                    <div class="flex flex-wrap gap-2 items-center">
                        ...
                        @endif
                            {{-- phpMyAdmin and "Import Database" buttons
                                 were removed per user request — they
                                 duplicate functionality already exposed
                                 at the service level (phpmyadmin is its
                                 own card in WordPress Manager, and the
                                 .sql import flow lives inside phpMyAdmin
                                 itself). Keep the openDatabasePanel and
                                 openImportDatabaseDialog Livewire methods
                                 in place so any other caller (modal,
                                 deep link) keeps working — only the
                                 toolbar buttons are gone. --}}
                        @if ($selected_container !== 'default')
                                    ...
                                    <tbody>
                                        @foreach ($files ?? [] as $file)
                                            ...
                                            <tr wire:key="file-row-{{ md5($filePath) }}" class="border-b border-coolgray-200 dark:border-coolgray-700 hover:bg-coolgray-50 dark:hover:bg-coolgray-800 {{ $isSelected ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}">
                                                <td class="p-2">
                                                    <input type="checkbox" wire:model.live="selectedFiles" value="{{ $filePath }}" class="cursor-pointer">
                                                </td>
                                                <td class="p-2">
                                                    <div class="flex items-center gap-2">
